...
prot=pd.read_csv('sp_emx1_only_protein_withlabels_filtered.csv')
prot=prot.drop(['Label','System','sys-labels'], axis=1)

# Step 1: Prepare the prot
# dropping the rows having NaN values
prot = prot.dropna()
 # To reset the indices
prot = prot.reset_index(drop=True)
X = prot.iloc[:, 1:]  # Remove the first five columns as they are identifiers and not relevant features
...
with open('Feature_Selector_prot_spcas9_filtered','wb') as f: pickle.dump(Feature_Selector, f)
